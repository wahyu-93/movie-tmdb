...
<head>
    ...
    <title>BeeCinemation | Detail Tv Show - {{ $detailTv->name }}</title>
...
<body>
    <div class="w-full h-screen flex flex-col relative">
        <img src="{{ $imageBaseURL }}/original{{ $detailTv->backdrop_path }}" alt="" class="w-full h-screen object-cover lg:object-fill absolute">
        ...
        <div class="w-full h-full z-10 flex flex-col justify-center px-20">
            <span class="text-white text-6xl font-quicksand font-bold">{{ $detailTv->name }}</span>
            <span class="text-white font-quicksand text-2xl italic max-w-3xl line-clamp-5 mt-4">{{ $detailTv->tagline == "" ? $detailTv->overview : $detailTv->tagline  }}</span>
            ...
            <div class="flex flex-row items-center mt-4">
                @php
                    ...
                    $offset = $circumference * ((100 - round($detailTv->vote_average * 10))/100);

                    $trailerId = '';
                    if($detailTv->videos->results){
                        foreach($detailTv->videos->results as $item){
                ...
                <div class="w-20 h-20 bg-red-100 rounded-full flex justify-center items-center mr-4" style="background: #00304D;">
                    <svg class="-rotate-90 w-20 h-20">
                        ...
                        <span class="text-white absolute font-inter font-bold text-xl">{{ round($detailTv->vote_average * 10) }}% </span>
                    </div>

                <div class="p-4 border border-white rounded-lg">
                    <span class="text-white text-2xl font-inter">{{ date_format(new DateTime($detailTv->first_air_date), "Y") }}</span>
                </div>

                @if($detailTv->episode_run_time)
                    <div class="mx-2 p-4 border border-white rounded-lg">
                        <span class="text-white text-2xl font-inter">{{ (int)($detailTv->episode_run_time[0]).'m / episode' }}</span>
                    </div>
                @endif
            </div>
